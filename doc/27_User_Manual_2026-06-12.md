...
- narrow the asset list with `Product`, `Status`, and `Search` filters before assigning a tag
...
- this baseline is currently a service/workflow seam, not yet a dedicated desktop operator screen
...
1. Open `Tags`
2. Reuse an existing `Tag Group` from the drop-down when it matches your intent, or type a new one if needed
3. Use `Product`, `Status`, and `Search` filters to narrow the asset list
4. Create the tags you want
5. Select one tag and one asset
6. Click `Assign Selected Tag`
