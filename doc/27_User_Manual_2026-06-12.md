...
- `Attach Role` now offers a suggested pick list for common roles and still allows typed custom roles when the workflow needs a special label
...
3. Choose a role such as `hero`, `hook`, `broll`, `cta`, `voice`, or `music` from the `Attach Role` list, or type a custom role if needed
