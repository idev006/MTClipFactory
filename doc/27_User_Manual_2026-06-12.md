...
- each asset folder may now also include `tags.toml` so folder-driven intake can assign normalized tags during the same automation run
- each product folder may now also include `captions.toml` so preview/final runtime can resolve main/sub caption overlays from product-level pools after the folder is processed once
