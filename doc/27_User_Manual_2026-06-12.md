...
- `Attach Role` now offers suggested choices based on the selected asset type and still allows typed custom roles when the workflow needs a special label
...
3. Choose a role suggested by the selected asset type, such as `hook`, `problem`, `benefit`, `proof`, `cta`, `hero`, `voice`, or `music`, or type a custom role if needed
