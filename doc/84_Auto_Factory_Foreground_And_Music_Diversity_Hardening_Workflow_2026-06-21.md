...
This document is the SSOT for the next Auto Factory anti-duplicate hardening slice that reduces repeated foreground and `background_music` reuse when fresh alternatives exist.

After [88_Auto_Factory_Persistent_Foreground_Background_Clip_Policy_2026-06-21.md](/F:/programming/python/MTClipFactory/doc/88_Auto_Factory_Persistent_Foreground_Background_Clip_Policy_2026-06-21.md), operator-visible foreground diversity now means clip-level foreground-asset diversity. The internal `foreground_sequence` planner axis remains as a compatibility representation for that persistent foreground choice.
...
- reduce batches that still look duplicate-like because they keep reusing the same clip-level foreground asset or background music
...
2. foreground variation may stay too concentrated on a small set of historically repeated clip-level foreground assets even when feasible fresh alternatives exist
...
- strengthen planner pressure against reused foreground assets and reused music when feasible alternatives exist

## Expected Behavior

When multiple feasible foreground assets or music assets exist:
...
- pytest locks regression cases for hidden music alternatives and historically repeated foreground reuse
