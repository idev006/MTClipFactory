...
| ISS-029 | 2026-06-12 | Medium | Theme Separation Debt | The UI standard now requires reusable theme assets instead of inline stylesheet strings, but `src/mt_clip_factory/ui/control_center/settings_window.py` still embeds `setStyleSheet(...)` and should be migrated to a theme-loading seam. | Engineering | Open | Extract the current settings window stylesheet into a reusable theme file or theme loader and add regression coverage for theme application. |
