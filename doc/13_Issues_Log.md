...
| ISS-032 | 2026-06-13 | High | Distributed Worker Orchestration | Scalable worker-node execution now depends on validating the delivered local-worker lease, heartbeat, retry, and idempotency baseline before the design expands into queue-backed or multi-node execution. | Engineering | Open | Validate the local-worker control baseline under broader operator use, then define the bounded queue and multi-worker extension seam. |
