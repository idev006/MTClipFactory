...
def test_auto_factory_window_shows_persisted_duplicate_risk_in_orders_tab(qapp: QApplication) -> None:
    ...
    assert "Risk Focus: Medium" in auto_factory_window.order_summary_text.toPlainText()
    assert "0.275" in auto_factory_window.order_summary_text.toPlainText()
    assert auto_factory_window.order_product_progress_table.item(0, 4).text() == "Medium"
    assert auto_factory_window.order_product_progress_table.item(0, 5).text() == "0.275"
    assert auto_factory_window.order_stages_table.item(0, 8).text() == "Medium"
    assert auto_factory_window.order_stages_table.item(0, 9).text() == "0.275"
    assert auto_factory_window.order_stages_table.item(0, 10).text() == "voice_asset_overused"
    auto_factory_window.close()


def test_auto_factory_window_filters_and_sorts_order_risk_rows(qapp: QApplication) -> None:
    selected_order = ProductionOrderDetailsDTO(
        production_order_id=9,
        order_code="tea_order_009",
        batch_code="tea_batch",
        source_mode="folder_control_surface",
        requested_by=None,
        strict_fulfillment=True,
        ...
        lease_expires_at=None,
        blocking_reason=None,
        created_at="2026-06-21 11:10:00",
        started_at="2026-06-21 11:10:01",
        finished_at="2026-06-21 11:10:07",
        items=(
            ProductionOrderItemDTO(
                production_order_item_id=5,
                product_id=1,
        ...
        stages=(
            ProductionOrderStageDTO(
                ...
                failure_class=None,
                detail_json='{"recipe_code":"tea_batch_001","near_duplicate_score":0.275,"near_duplicate_reasons":["voice_asset_overused"]}',
                created_at="2026-06-21 11:10:02",
                updated_at="2026-06-21 11:10:02",
            ),
            ProductionOrderStageDTO(
                ...
                output_id=None,
                failure_class=None,
                detail_json='{"recipe_code":"cream_batch_001","near_duplicate_score":0.775,"near_duplicate_reasons":["foreground_sequence_reused","voice_asset_overused"]}',
                created_at="2026-06-21 11:10:03",
                updated_at="2026-06-21 11:10:03",
            ),
            ProductionOrderStageDTO(
                ...
                failure_class=None,
                detail_json=None,
                created_at="2026-06-21 11:10:05",
                updated_at="2026-06-21 11:10:05",
            ),
        ),
        events=(),
    )
    ...
    auto_factory_window._refresh_selected_order()
    auto_factory_window.order_risk_filter_combo.setCurrentIndex(1)

    assert auto_factory_window.order_product_progress_table.rowCount() == 1
    assert auto_factory_window.order_product_progress_table.item(0, 0).text() == "cream"
    assert auto_factory_window.order_stages_table.rowCount() == 1
    assert auto_factory_window.order_stages_table.item(0, 8).text() == "High"

    auto_factory_window.order_risk_filter_combo.setCurrentIndex(0)
    auto_factory_window.order_stage_sort_combo.setCurrentIndex(1)

    assert auto_factory_window.order_stages_table.rowCount() == 3
