...
def test_stage_rows_do_not_claim_render_duplicate_risk_when_render_score_is_zero() -> None:
    order = ProductionOrderDetailsDTO(
        production_order_id=21,
        order_code="tea_order_021",
        batch_code="tea_batch_021",
        source_mode="folder_control_surface",
        requested_by=None,
        strict_fulfillment=True,
        preview_generation_enabled=True,
        ...
        started_at="2026-06-26 11:00:01",
        finished_at="2026-06-26 11:00:03",
        items=(
            ProductionOrderItemDTO(
                production_order_item_id=5,
                product_id=1,
        ...
        stages=(
            ProductionOrderStageDTO(
                ...
                detail_json=(
                    '{"recipe_code":"tea_batch_001","output_code":"preview_001","clip_formula_hash":"abc123def4567890",'
                    '"history_scope":"auto_factory_preview","duplicate_risk":0.0,"review_signal_codes":[]}'
                ),
                created_at="2026-06-26 11:00:02",
                updated_at="2026-06-26 11:00:02",
            ),
        ),
        events=(
            ProductionOrderEventDTO(
                ...
                event_type="run_completed",
                status="succeeded",
                message="Completed production order tea_order_021 with status succeeded.",
                production_order_item_id=None,
                stage_name=None,
                worker_id=None,
                detail_json=None,
                created_at="2026-06-26 11:00:03",
            ),
        ),
    )

    row = build_order_stage_rows(order)[0]

    assert row.risk_level == "Low"
    assert row.risk_score == 0.0
    assert "history_scope:auto_factory_preview" in row.risk_reasons
    assert "clip_formula_hash:abc123def456" in row.risk_reasons
    assert "render_duplicate_risk" not in row.risk_reasons


def test_order_summary_surfaces_preset_spread_and_requested_preset_truth() -> None:
    order = ProductionOrderDetailsDTO(
        production_order_id=22,
        order_code="tea_order_022",
        batch_code="tea_batch_022",
        source_mode="folder_control_surface",
        requested_by=None,
        strict_fulfillment=True,
        preview_generation_enabled=True,
        ...
        started_at="2026-06-27 12:00:01",
        finished_at="2026-06-27 12:00:08",
        items=(
            ProductionOrderItemDTO(
                production_order_item_id=6,
                product_id=1,
                product_code="tea",
                ...
                creative_preset_codes=("presenter_urgency", "proof_pack_trust"),
            ),
        ),
        stages=(
            ProductionOrderStageDTO(
                production_order_stage_id=21,
                stage_name="materialize",
                stage_scope="recipe",
                status="succeeded",
                sequence_index=1,
                ...
                failure_class=None,
                detail_json=(
                    '{"recipe_code":"tea_batch_022_001","near_duplicate_score":0.240,'
                    '"creative_preset_code":"presenter_urgency",'
                    '"creative_preset_signature":"presenter_urgency|hook_benefit_cta|dense|hook,cta|sale_blast|dark_lower_third"}'
                ),
                created_at="2026-06-27 12:00:02",
                updated_at="2026-06-27 12:00:02",
            ),
            ProductionOrderStageDTO(
                production_order_stage_id=22,
                stage_name="materialize",
                stage_scope="recipe",
                status="succeeded",
                sequence_index=2,
                ...
                failure_class=None,
                detail_json=(
                    '{"recipe_code":"tea_batch_022_002","near_duplicate_score":0.255,'
                    '"creative_preset_code":"presenter_urgency",'
                    '"creative_preset_signature":"presenter_urgency|hook_benefit_cta|dense|hook,cta|sale_blast|dark_lower_third"}'
                ),
                created_at="2026-06-27 12:00:03",
                updated_at="2026-06-27 12:00:03",
            ),
            ProductionOrderStageDTO(
                production_order_stage_id=23,
                stage_name="materialize",
                stage_scope="recipe",
                status="succeeded",
                sequence_index=3,
                ...
                failure_class=None,
                detail_json=(
                    '{"recipe_code":"tea_batch_022_003","near_duplicate_score":0.180,'
                    '"creative_preset_code":"proof_pack_trust",'
                    '"creative_preset_signature":"proof_pack_trust|proof_focus|medium|proof,benefit|benefit_stack|dark_lower_third"}'
                ),
                created_at="2026-06-27 12:00:04",
                updated_at="2026-06-27 12:00:04",
            ),
        ),
        events=(),
    )
    ...
    assert "Requested Preset Modes: balanced_cycle" in summary
    assert "Requested Preset Codes: presenter_urgency, proof_pack_trust" in summary
    assert "Creative Presets: presenter_urgency, proof_pack_trust" in summary
    assert "Preset Spread: presenter_urgency x2 (67%), proof_pack_trust x1 (33%)" in summary
    assert "Preset Concentration: unique=2, max_share=0.667, recipes=3" in summary
